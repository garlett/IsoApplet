...
public class IsoApplet extends Applet implements ExtendedLength {
    ...
    */
    ...
    private short ram_buf_insert_tag(short pos, short tag, short datalen) throws InvalidArgumentsException, NotEnoughSpaceException {
 
	Util.arrayCopy( ram_buf, pos, ram_buf, (short) (pos + 1 + UtilTLV.getLengthFieldLength(datalen)), datalen );
	return (short) (pos + datalen + UtilTLV.writeTagAndLen(tag, datalen, ram_buf, pos));
   }
	...
	pos = ram_buf_insert_tag(pos, (short)0x92, key.getP(ram_buf, pos));         // P

	pos = ram_buf_insert_tag(pos, (short)0x93, key.getQ(ram_buf, pos));         // Q

	pos = ram_buf_insert_tag(pos, (short)0x94, key.getPQ(ram_buf, pos));         // PQ (1/q mod p)

	pos = ram_buf_insert_tag(pos, (short)0x95, key.getDP1(ram_buf, pos));        // DP1 (d mod (p-1))
    ...
    private short ecprivkey_to_iso7816_at_globalrambuff(ECPrivateKey key) throws InvalidArgumentsException, NotEnoughSpaceException {

        short pos = 5; // 0x7F48

	pos = ram_buf_insert_tag(pos, (short)0x91, key.getS(ram_buf, pos));        // Private key

	//pos = ram_buf_insert_tag(pos, (short)0x06, key.getO(ram_buf, pos));        // Object identifier of related curve (optional)
	//        Curve information (conditional if DO'06' above is not specified)

	pos = ram_buf_insert_tag(pos, (short)0x93, key.getField(ram_buf, pos));        // p is the prime specifying the base field

	pos = ram_buf_insert_tag(pos, (short)0x94, key.getA(ram_buf, pos));        // A 1st coefficient of the equation y^2 = x^3 + A*x + B mod p defining the elliptic curve

	pos = ram_buf_insert_tag(pos, (short)0x95, key.getB(ram_buf, pos));        // B 2nd coefficient of the equation y^2 = x^3 + A*x + B mod p

	pos = ram_buf_insert_tag(pos, (short)0x96, key.getG(ram_buf, pos));        // G = (x,y) base point, i.e. a point in E of prime order, with x and y being its x- and y-coordinates

	pos = ram_buf_insert_tag(pos, (short)0x97, key.getR(ram_buf, pos));        // q prime order of the group generated by G
...
//			len = rsaprivkey_to_iso7816_at_globalrambuff( (RSAPrivateCrtKey) keys[currentPrivateKeyRef[0]] );
		...
		Util.arrayFillNonAtomic(ram_buf, (short)0, (short)(RAM_BUF_SIZE-1), (byte)0x00);
